...
function canResolveApproval(client: GatewayClient | null): boolean {
  ...
  return isTrustedApprovalRuntime || canReviewApproval(client);
}
...
function loadVisibleApproval(params: {
  ...
  const authorized = params.allowApprovalRuntime
    ? canResolveApproval(params.client)
    : canReviewApproval(params.client);
  if (!authorized) {
    return null;
  }
